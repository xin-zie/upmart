...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function switchMode(isSeller) {
        ...
        if (isSeller) {
            ...
            el.dynamicLink.innerHTML = '<span class="icon">𖠩</span> My Orders';
   ...
   function loadThread() {
        ...
        fetch(`handle_actions.php?get_messages=1&product_id=${pid}&other_user=${activeSellerId}`)
            .then(r => {
                // If the server sends a 500 error or HTML, don't parse it
    ...
    window.openPostReportModal = function (productId, productName) {
        document.getElementById('postReportLabel').textContent = 'Product: ' + productName;
