...
if (strlen($plain_password) < 8) {
    header("Location: reset_pass.php?token=$token&error=short");
    exit();
} 

// Check for Letter/Number requirement
if (!preg_match("/[a-z]/i", $plain_password) || !preg_match("/[0-9]/", $plain_password)) {
    header("Location: reset_pass.php?token=$token&error=format");
    exit();
}

// Check for Mismatch
if ($plain_password !== $confirm_password) {
    header("Location: reset_pass.php?token=$token&error=match");
    exit();
}
...
if ($stmt->execute()) {
    ...
    header("Location: ../index.php?reset=complete");
    exit();
} else {
